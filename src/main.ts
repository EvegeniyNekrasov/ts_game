...
function handleInput(dt: number) {
  vx = 0;
  vy = 0;
  if (input.actionDown("left")) vx -= speed;
  if (input.actionDown("right")) vx += speed;
  if (input.actionDown("up")) vy -= speed;
  if (input.actionDown("down")) vy += speed;
